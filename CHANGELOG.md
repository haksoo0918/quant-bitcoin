...
### Added (추가됨)
- **대시보드 메인 전략(50:50 듀얼 모멘텀) 요약 설명문 추가 (`docs/index.html`, `PRD.md`, `docs/service-worker.js`)**:
  - 서브 전략과 일관된 형태로 메인 전략 섹션 타이틀 하단에 핵심 운용 원리(BTC 220일 SMA 버퍼 & ETH 50일 SMA ATR 채널 개별 추세 추종, 상승장 50:50 균형 분할 투자, 하락장 안전 현금화, 비중 편차 ±10%p 발생 시 자동 리밸런싱)를 직관적으로 안내하는 설명문(`.section-desc`) 추가.
  - 서비스 워커 캐시 버전(`v1.8.11`) 갱신으로 PWA 대시보드 즉시 동기화.
...
### Changed (변경됨)
- **모바일 뷰포트 헤더 액션 버튼 아이콘 단독 뷰 최적화 (`docs/index.html`, `PRD.md`, `docs/service-worker.js`)**:
  - 모바일(`<= 640px`) 환경에서 다크모드 및 실시간 시세 버튼의 텍스트를 숨기고 컴팩트한 아이콘 단독 버튼(`34px`/`32px`)으로 전환.
  - 핵심 CTA인 PWA 앱 설치 버튼은 모바일에서도 다운로드 아이콘과 함께 '앱 설치' 텍스트를 지속 노출하여 가시성과 설치 전환율 확보.
  - 서비스 워커 캐시 버전(`v1.8.8`) 갱신.

## [1.8.7] - 2026-09-04

### Changed (변경됨)
- **대시보드 종합 매매 행동 가이드 시각 상태 기호 통일 (`docs/index.html`, `PRD.md`, `docs/service-worker.js`)**:
  - 메인 전략(BTC / ETH) 행동 요약에 각 코인의 신호 판정(상승 매수 `🟢` / 하락 현금화 `🔴` / 중립 버퍼 `🟡`) 상태 원형 기호 표기 추가.
  - 서브 전략과 동일한 시각적 기호 체계(`🟢 / 🔴 / 🟡`)를 적용하여 당일 포트폴리오 포지션을 즉시 식별 가능하도록 UX 개선.
  - 서비스 워커 캐시 버전(`v1.8.7`) 갱신.

## [1.8.6] - 2026-09-02

### Added (추가됨)
- **대시보드 백테스트 탭 인터페이스 및 전략별 누적 자산 성장 곡선 차트 이미지 연동 (`docs/index.html`, `docs/images/`, `docs/service-worker.js`, `PRD.md`)**:
  - `[메인 전략 (BTC/ETH 50:50)]` & `[서브 전략 (이더리움 SuperTrend)]` 백테스트 탭 전환 UI 구현.
  - 각 전략의 1,500일(4.1~4.3년) 시뮬레이션 상세 모델 사양 및 기간 정보 표기.
  - 퀀트 전략 vs 벤치마크(단순보유) 누적 자산 성장 곡선 및 낙폭 분석 그래프 이미지(`docs/images/backtest_main.png`, `docs/images/backtest_sub_eth.png`) 연동.
  - 서비스 워커 캐시 버전(`v1.8.6`) 갱신 및 오프라인 이미지 캐싱 지원.
