# -*- coding: utf-8 -*-
"""
서브 전략: 알트코인 상대 모멘텀 백테스트 시뮬레이터 (Altcoin Momentum Backtester)
- 공통 시장 필터: 업비트 BTC 220일 SMA ±2% 노이즈 버퍼 (하락장 시 100% 현금화)
- 주간 리밸런싱: 매주 월요일 09:00, 최근 7일 거래대금 TOP 10 중 14일 수익률 TOP 4 종목 선정 (각 25% 균등 매수)
"""
import os
import sys
import time
...
def run_altcoin_backtest(days=1000, initial_capital=10000000.0, btc_sma_len=220, buffer_rate=0.02,
                         momentum_window=14, volume_window=7, top_volume_count=10, select_count=4,
                         fee_rate=0.0004, slippage=0.0005):
    """
    알트코인 상대 모멘텀 전략 시뮬레이션
    """
    print(f"\n{'='*70}")
    print(f"📊 [서브 전략] 빗썸 알트코인 상대 모멘텀 백테스트 시작")
    print(f"• 분석 기간: 최근 {days}일 | 초기 자본: {initial_capital:,.0f} 원")
    print(f"• 공통 시장 필터: 업비트 BTC {btc_sma_len}일 SMA (±{buffer_rate*100:.1f}% 노이즈 버퍼)")
    print(f"• 종목 선정: 최근 {volume_window}일 거래대금 상위 {top_volume_count}선 중 {momentum_window}일 수익률 TOP {select_count}")
    ...
    # 2. 알트코인 유니버스 데이터 수집
    print(f"▶ 2. 알트코인 유니버스 ({len(DEFAULT_ALTCOIN_UNIVERSE)}개 종목) 과거 일봉 수집 및 전처리...")
    alt_dfs = {}
    for sym in DEFAULT_ALTCOIN_UNIVERSE:
        df = fetch_and_cache_candles(sym, days_needed=days + 50)
        if df is not None and len(df) >= 100:
            alt_dfs[sym] = df
            print(f"  ✓ {sym}: {len(df)}일 데이터 로드 완료")
        else:
            print(f"  ⚠️ {sym}: 데이터 부족으로 제외")
    ...
    # 최근 days 일간의 테스트 날짜 추출
    test_dates = btc_df['date'].iloc[-days:].reset_index(drop=True)
    
    # 4. 시뮬레이션 상태 변수 초기화
    cash = float(initial_capital)
    holdings = {} # { 'KRW-XRP': { 'qty': float, 'buy_price': float, 'buy_date': date } }
    equity_history = []
    trade_logs = []
    ...
    print(f"\n▶ 3. 일별 시뮬레이션 구동 시작 (총 {len(test_dates)}일간)...")

    # 날짜별 가격 빠른 조회를 위한 맵 구성
    price_maps = {}
    ...
    for current_date in test_dates:
        ...
        if btc_price >= btc_upper:
            ...
            market_filter_state = "Bear"
        # 버퍼 구간이면 기존 market_filter_state 유지

        is_monday = (current_date.weekday() == 0)

        # B. 하락장(Bear) 전환 시 전량 매도 (현금화)
        ...
        # C. 상승장(Bull) 시 주간 리밸런싱 (월요일 09:00 또는 Bear->Bull 전환 첫 진입)
        elif market_filter_state == "Bull" and (is_monday or len(holdings) == 0):
            # 1) 전체 알트코인 모멘텀 및 거래대금 연산
            candidates = []
            for sym, df_coin in price_maps.items():
                if current_date in df_coin.index:
                    hist = df_coin.loc[:current_date]
                    if len(hist) >= max(volume_window, momentum_window) + 1:
                        avg_val = hist['value'].iloc[-volume_window:].mean()
                        ret_14d = (hist['close'].iloc[-1] - hist['close'].iloc[-momentum_window-1]) / hist['close'].iloc[-momentum_window-1]
                        candidates.append({
                            'symbol': sym,
                            'avg_val_7d': avg_val,
                            'return_14d': ret_14d,
            ...
            if len(candidates) >= select_count:
                ...
                target_coins = [c['symbol'] for c in top_vol[:select_count]]
    ...
    # 콘솔 출력
    print("\n" + "="*70)
    print("🎯 [서브 전략: 빗썸 알트코인 상대 모멘텀 백테스트 결과]")
    print("="*70)
...
def generate_altcoin_chart(df, save_path, metrics):
    """
    알트코인 백테스트 자산 곡선 및 낙폭 차트 시각화
    """
    plt.rcParams['font.family'] = 'DejaVu Sans'
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True, gridspec_kw={'height_ratios': [2.5, 1]})

    # 1. 상단: 누적 수익률 비교
    ax1.plot(df['date'], (df['equity'] / metrics['initial_capital']) * 100, label=f"Altcoin Momentum Strategy (CAGR {metrics['cagr']:+.1f}%)", color='#0284c7', linewidth=2.0)
    ...
    ax1.set_title("Altcoin Relative Momentum Strategy vs BTC Buy & Hold", fontsize=14, fontweight='bold', pad=12)
...
def generate_altcoin_report(df, trade_logs, metrics, plot_filename, save_path):
    """
    한글 볼드 띄어쓰기 표준을 엄격히 준수한 백테스트 마크다운 보고서 생성
    """
    report = []
    report.append(f"# 📊 [서브 전략] 빗썸 알트코인 상대 모멘텀 백테스트 성과 보고서\n")
    ...
    report.append(f"| 평가 지표 | 알트코인 상대 모멘텀 전략 | BTC 단순 보유 (Buy & Hold) | 초과 성과 |")
    report.append(f"| :--- | :---: | :---: | :---: |")
    ...
    report.append(f"> 💡 **주요 성과 요약**: BTC 단순 보유 대비 **{outperformance:+.2f}%p** 의 초과 수익률을 달성하였으며, 시장 필터(BTC 220 SMA)를 통한 하락장 100% 현금화로 벤치마크 대비 **{mdd_defense:.2f}%p** 만큼의 최대 낙폭(MDD) 방어 효과를 기록하였습니다.\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="빗썸 알트코인 상대 모멘텀 퀀트 전략 백테스터")
