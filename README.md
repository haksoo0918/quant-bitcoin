# 📈 암호화폐 퀀트 자동매매 & PWA 웹 대시보드 시스템

이 프로젝트는 지정 거래소(업비트 & 빗썸) API를 활용하여 검증된 퀀트 전략을 바탕으로 매일 오전 09:05(KST) 포트폴리오의 리밸런싱 방향을 분석·자동 집행하고, 체결 리포트를 디스코드로 전송하는 독립형 암호화폐 퀀트 트레이딩 시스템입니다.

또한, 모바일 및 데스크톱 환경에서 당일 매매 가이드, 최근 60일 인터랙티브 지표 차트, 1,500일 역사적 백테스트 자산 성장 곡선을 한눈에 확인할 수 있는 독립형 **PWA 웹 대시보드**를 함께 제공합니다.

### 🌟 핵심 시스템 구성
1. **메인 전략 (업비트)**: BTC(220일 SMA ±2% 버퍼) & ETH(50일 SMA ± 1.5 ATR) 50:50 듀얼 모멘텀 비중 조절 포트폴리오
2. **서브 전략 (빗썸)**: 이더리움 단독 SuperTrend(7, 3.5) & 50일 SMA 복합 대세 추세 추종 (100% 매수 / 100% 현금화)
3. **PWA 웹 대시보드**: 오전 9시 확정 일봉 기준 0.01초 즉시 렌더링, 최근 60일 지표 시각화 Canvas 차트, 1,500일 역사적 백테스트 2종 비교 뷰
